...
m1_by_flight_df = pd.DataFrame(columns=['flightId', 'beginTimestamp', 'endTimestamp'])
# Keep the actual date and time (not planned)
m3_by_flight_df = pd.DataFrame(columns=['flightId', 'endDate', 'endTime', 'endHour', 'beginTimestamp', 'endTimestamp'])
...
for id, new_df in m1_df.groupby(level='flightId'):
    ...
    m1_by_flight_df = m1_by_flight_df.append({'flightId': id, 'beginTimestamp': begin_timestamp, 'endTimestamp': end_timestamp}, ignore_index=True)
...
for id, new_df in m3_df.groupby(level='flightId'):
    ...
    end_date = new_df['endDate'].values[-1]
    end_time = new_df['endTime'].values[-1]
    end_hour = int(int(end_time)/10000)
    
    m3_by_flight_df = m3_by_flight_df.append({'flightId': id, 'endDate':  end_date, 'endTime': end_time, 'endHour': end_hour, 
                                              'beginTimestamp': begin_timestamp, 'endTimestamp': end_timestamp}, ignore_index=True)

print("ddr stat")

stat_by_flight_df = pd.merge(m1_by_flight_df, m3_by_flight_df, on=['flightId'], suffixes=["_L", "_R"])
...
stat_by_flight_df = stat_by_flight_df[['endDate', 'endTime', 'endHour', 'flightId', 'departure_delay', 'arrival_delay', 'enroute_delay']]
