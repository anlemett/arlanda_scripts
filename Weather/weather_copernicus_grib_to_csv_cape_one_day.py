...
for h in range(0,24):
    selected_grbs=np.array(grbs.select(name='Convective available potential energy', hour=h, minute=0, second=0))
...
data_df = pd.DataFrame(new_data, columns = ['month', 'day', 'hour', 'lat', 'lon', 'cape'])
