...
for month in range(1,13):
    ...
    for d in range(1,number_of_days):
        ...
        for h in range(0,24):
            data_all=np.array(grbs.select(name='Snow density', hour=h, minute=0, second=0))[0]

            snow = data_all.data(lat1=my_y1,lat2=my_y2,lon1=my_x1,lon2=my_x2)
            #print(snow)
            for lat in range(8,-1,-1):
                for lon in range (8,-1,-1):
                    new_d = {}
                    new_d['time'] = h
                    new_d['lat'] = snow[1][lat][0]
                    new_d['lon'] = snow[2][0][lon]
                    new_d['snow'] = snow[0][lat][lon]
            
                    new_data.append(new_d)
                    #print(h, snow[1][lat][0], snow[2][0][lon], snow[0][lat][lon])

data_df = pd.DataFrame(new_data, columns = ['time', 'lat', 'lon', 'snow'])
