...
nc_filename = 'data/weather_copernicus_TMA_grib_2018/copernicus_TMA_cloud_2018.nc'
...
df = df[['month','day','hour', 'latitude', 'longitude', 'tcc']]
df.rename(columns={'latitude': 'lat', 'longitude': 'lon'}, inplace=True)

df = df.sort_values(by = ['month', 'day', 'hour', 'lat', 'lon'], ascending = [True, True, True, True, False])

df.to_csv('data/weather_copernicus_TMA_csv_2018/copernicus_TMA_cloud_2018.csv', sep=' ', encoding='utf-8', float_format='%.3f', header=True, index=True)
